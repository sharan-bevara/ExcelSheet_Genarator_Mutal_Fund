...
def build_csv_export(df):
    """Build CSV export exactly matching the sample format with 5-row header block."""
    export_cols = [
        "Fund Name","AUM Cr.","TER","PE","PB",
        "Top 3 Holdings","Top 5 Holdings","Top 10 Holdings","Top 20 Holdings",
        "Sharpe","Sortino","St Dev","Inception","Age in Yrs","score","Rank"
    ]
    higher_lower = ["Higher/lower","higher","lower","lower","lower","lower","lower","lower","lower","higher","higher","lower","higher","higher","",""]
    weightage    = ["Weightage","9","8","7","7","6","6","6","6","15","13","9","3","5","score","Rank"]

    lines = []
    # Row 1: title
    lines.append("NGEN Markets Filter Results: " + "," * (len(export_cols)-1))
    # Row 2: blank with Notes
    blank_row = [""] * len(export_cols)
    blank_row[-1] = "Notes"
    lines.append(",".join(blank_row))
    # Row 3: column headers
    lines.append(",".join(export_cols))
    # Row 4: higher/lower
    lines.append(",".join(higher_lower[:len(export_cols)]))
    # Row 5: weightage
    lines.append(",".join(weightage[:len(export_cols)]))

    # Data rows
    out = df.rename(columns={"Score": "score"})[
        [c for c in export_cols if c in df.columns or c == "score"]
    ].copy()
    # Rename Score -> score
...
if run_clicked and st.session_state.funds_df is not None:
    ...
    st.session_state.logs.append(
        f"[{ts()}] ▶ Starting  —  {total:,} funds  ·  {batches} batches  ·  model={model}")
    st.session_state.logs.append(
    ...
    for idx in range(batches):
        ...
        if idx < batches - 1 and st.session_state.running:
            time.sleep(delay)
...
{mcard("🥇 Rank 1", (top_fund[:35]+"…") if len(top_fund)>35 else top_fund)}
    ...
    # Download — exact CSV format matching sample
    csv_str = build_csv_export(df)
    st.download_button(
        "⬇️  Download Rankings_Master.csv",
        csv_str, "Rankings_Master.csv", "text/csv",
        use_container_width=True,
    )
